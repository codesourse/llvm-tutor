...
namespace CodeReview {
    ...
    public:
        ...
        bool isUserSourceCode(const string filename) {
            ...
            if (filename.find("/Applications/Xcode13.4.app/") == 0) return false;
            return  true;
        }
        ...
        void run(const MatchFinder::MatchResult &Result) {
            ...
            if (const ObjCInterfaceDecl *decl = Result.Nodes.getNodeAs<ObjCInterfaceDecl>("ObjCInterfaceDecl")) {
                ...
                if (pos != StringRef::npos) {
                    ...
                    D.Report(loc, D.getCustomDiagID(DiagnosticsEngine::Warning, "CodeReview：类名中不能带有下划线"));
                }
            }
            ...
            if (const VarDecl *decl = Result.Nodes.getNodeAs<VarDecl>("VarDecl")) {
                ...
                if (pos != StringRef::npos && pos != 0) {
                    DiagnosticsEngine &D = ci.getDiagnostics();
                    SourceLocation loc = decl->getLocation().getLocWithOffset(pos);
                    D.Report(loc, D.getCustomDiagID(DiagnosticsEngine::Warning, "CodeReview2：请使用驼峰命名，不建议使用下划线"));
                }
            }
        }
    };
    ...
    public:
        ...
        void HandleTranslationUnit(ASTContext &Ctx) {
            printf("CodeReview1: All ASTs has parsed.");
            DiagnosticsEngine &D = Ctx.getDiagnostics();
            // 在编译log中可以看到
            //D.Report(D.getCustomDiagID(DiagnosticsEngine::Warning, "CodeReview警告提示"));
           // D.Report(D.getCustomDiagID(DiagnosticsEngine::Error, "CodeReview错误信息"));
...
static FrontendPluginRegistry::Add<CodeReview::TestAction>
X("CodeReview", "Test a new Plugin");
// X 变量名，可随便写，也可以写自己有意思的名称
// CodeReview  插件名称，️很重要，这个是对外的名称
